...
if (!defined('_PS_VERSION_'))
	exit;
include_once(dirname(__FILE__).'/classes/ybc_widget_widget_class.php');
class Ybc_widget extends Module
{
	...
	{	   
        ...
        $this->_hooks = array(
            ...
            array(
                'hook' => 'ybcCustom3',
                'name' => $this->l('Hook Custom services')
            ),
            array(
                'hook' => 'ybcCustom4',
                'name' => $this->l('Hook Custom link on nav')
            ),
        );
    ...
    { 
        ...
        foreach($dirs as $dir)
        {
            ...
            foreach($files as $file){ 
              if(is_file($file) && $file!=dirname(__FILE__).'/images/'.$dir.'/index.php')
                @unlink($file); 
            }
        }        
        return true;
    ...
    public function getContent()
	{
       ...
       return '<div class="panel">'.$this->_html.'</div>'.$this->displayIframe();
    }
       ...
       if($oldFiles = glob(dirname(__FILE__).'/images/widget/*'))
        {
            foreach($oldFiles as $file){ 
              if(is_file($file) && $file!=dirname(__FILE__).'/images/widget/index.php')
                @unlink($file); 
            }
        }         
